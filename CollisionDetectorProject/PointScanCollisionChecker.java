...
//of the arm robot and the obstacles in the world; uses point scan to check
//time to collision of all points on robot arm
public class PointScanCollisionChecker {
	...
	public PointScanCollisionChecker(ArmRobot ar, ArrayList<Obstacle> obs) {
		armRobot = ar;
		obstacles = obs;
	...
	//returns the minimum time needed to collide into obstacle looking at all points on arm and their respective
	//minimum distance to collision and maximum velocity at that configuration
	public double getTMin() {
		...
		for (int pointDistance = 0; pointDistance <= armRobot.getArm1Length() + armRobot.getArm2Length(); pointDistance++) {
			...
			//System.out.println("minDistance:" + getMinDistanceOfPoint(armRobot.getXYofPoint(pointDistance)));
			//System.out.println("vMax:" + calculatePointVMax(pointDistance, 1));
			currentTime = getMinDistanceOfPoint(armRobot.getXYofPoint(pointDistance))/calculatePointVMax(pointDistance, 0);
			//System.out.println("time:" + currentTime);
			if (currentTime < tMin) {
				tMin = currentTime;
			}
		}
		System.out.println("PointScanCollisionChecker tMin: " + tMin);
		return tMin;
	}
	...
	public double calculatePointVMax(int distanceAlongArms, double time) {
		...
		if (distanceAlongArms > armRobot.getArm1Length() && distanceAlongArms <= armRobot.getArm1Length() + armRobot.getArm2Length()) {
			...
			//System.out.println(directDistanceToPoint + ", " + directAngleToPoint + ", " + arm1VelocityVectorAngle + ", " + arm2VelocityVectorAngle + ", " + arm1VelocityVectorMagnitude+ ", " + arm2VelocityVectorMagnitude);
